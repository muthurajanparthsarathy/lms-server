...
/**
 * resolveQuestionWithExercise({ nodeType, nodeId, category, subcategory, exerciseId, questionId })
 *
 * @returns {{question: object, exercise: object}|null} the raw question sub-document (with testCases[],
 *                         solutions.functionName, score/points, questionType),
 *                         or null if any lookup step fails.
 */
async function resolveQuestionWithExercise({
  nodeType,
  nodeId,
  category,
  subcategory,
  ...
  // Returned as a pair (rather than mutating the Mongoose subdocument) so
  // callers can read exercise-level settings — notably `evaluationMethod` —
  // without a second lookup.
  return { question, exercise: located.exercise };
}

/**
 * Back-compat wrapper: the question sub-document alone, or null.
 */
async function resolveQuestion(args) {
  const found = await resolveQuestionWithExercise(args);
...
async function loadForJudge(args) {
  ...
  const { question: q, exercise } = found;
  return {
    testCases: Array.isArray(q.testCases) ? q.testCases : [],
    functionName: (q.solutions && q.solutions.functionName) || null,
    maxMarks: Number(q.score ?? q.points ?? 10) || 10,
    questionType: q.questionType || null,
    // Exercise-level "how is this evaluated?" setting. The judge only runs
    // for Test Case exercises — Manual must never auto-score, and AI scores
    // are computed client-side and must not be overwritten. See
    // `shouldServerJudge` in controllers/courses/moduleStructure/answer.js.
    evaluationMethod: (exercise && exercise.evaluationMethod) || null,
    raw: q,
  };
}

module.exports = { resolveQuestion, resolveQuestionWithExercise, loadForJudge };
