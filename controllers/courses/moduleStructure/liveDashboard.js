...
async function resolveExercise(exerciseId, nodeType, nodeId) {
  const tryDoc = (doc) => {
    ...
    for (const [subcategory, exercises] of sectionEntries(youdo)) {
  ...
  if (nodeType && nodeId && MODEL_BY_TYPE[nodeType]) {
    const doc = await MODEL_BY_TYPE[nodeType].findById(nodeId).select("title pedagogy.You_Do").lean();
  ...
  for (const Model of ALL_MODELS) {
    const docs = await Model.find({ "pedagogy.You_Do": { $exists: true } })
      .select("title pedagogy.You_Do")
      .lean();
...
exports.getLiveDashboard = async (req, res) => {
  try {
    ...
    const course = await CourseStructure.findById(courseId)
      .select("courseName singleParticipants")
