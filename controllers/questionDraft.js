...
// POST /draft/save
// Body: { exerciseId, questionId, language?, files: [{ path, content }] }
// Auth: userAuth — userId comes from req.user, never the body.
exports.saveDraft = async (req, res) => {
  try {
    const userId = req.user?._id;
    const { exerciseId, questionId, language, files } = req.body || {};
...
exports.loadDraft = async (req, res) => {
  try {
    ...
    if (!doc) {
      return res.status(200).json({ success: true, draft: { files: [] } });
    }
    return res.status(200).json({ success: true, draft: doc });
  } catch (err) {
